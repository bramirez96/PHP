...
function make_form($id, $db) {
	$sql = "SELECT MAX(q_num), title FROM brandon.2601166_entity_surveys ES
				INNER JOIN brandon.2601166_xref_surveys_questions XSQ
	...
	for ($i = 1; $i <= $maxQ; $i++) {
		...
		$query = "SELECT question_id, q_num, question, q_type, survey_id FROM brandon.2601166_entity_surveys ES
						INNER JOIN brandon.2601166_xref_surveys_questions XSQ
							ON ES.id = XSQ.survey_id
						INNER JOIN brandon.2601166_entity_questions EQ
							ON XSQ.question_id = EQ.id
						INNER JOIN brandon.2601166_enum_q_types EQT
		...
		$result->free();
		$query = "SELECT MAX(a_num) FROM brandon.2601166_entity_surveys ES
						INNER JOIN brandon.2601166_xref_surveys_questions XSQ
							ON ES.id = XSQ.survey_id
						INNER JOIN brandon.2601166_entity_questions EQ
							ON XSQ.question_id = EQ.id
						INNER JOIN brandon.2601166_xref_questions_answers XQA
		...
		$result->free();
		for ($c = 1; $c <= $maxAns; $c++) {
			$query = "SELECT a_num, answer, answer_id FROM brandon.2601166_entity_surveys ES
							INNER JOIN brandon.2601166_xref_surveys_questions XSQ
								ON ES.id = XSQ.survey_id
							INNER JOIN brandon.2601166_entity_questions EQ
								ON XSQ.question_id = EQ.id
